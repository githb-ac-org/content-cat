...
export default function CreateCharacter() {
  ...
  const [deleteCharacterId, setDeleteCharacterId] = useState<string | null>(
    null
  );
  ...
  const handleSaveEdit = async (
    ...
    try {
      ...
      const updateResponse = await fetch(`/api/characters/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name,
  ...
  const handleGenerateWithCharacter = (characterId: string) => {
    router.push(`/image?model=characters&characterId=${characterId}`);
  };
              ...
              {characters.map((character) => (
                <div key={character.id} className="group relative">
                  <div className="relative h-72 w-48 overflow-hidden bg-zinc-800">
                    ...
                    <button
                      type="button"
                      onClick={() => handleGenerateWithCharacter(character.id)}
                    ...
                    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-3">
                      ...
                      <p className="text-xs text-gray-400">
                        {character.referenceImages.length} images
                      </p>
                    </div>
                  </div>
                </div>
